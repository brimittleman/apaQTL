...
```{r}
ggplot(PAS500res,aes(x=pheno, y=Prop._h2,fill=pheno)) + geom_bar(stat="identity") + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + theme(legend.position = "none") + annotate("text", label=paste("% of SNPs included", PAS500_snpsincluded), x=10, y=.08)


ggplot(PAS500res,aes(x=pheno, y=Enrichment, fill=pheno)) + geom_bar(stat="identity") + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + theme(legend.position = "none") + annotate("text", label=paste("% of SNPs included", PAS500_snpsincluded), x=10, y=12)
```

Plot the H2 explained: 

```{r}
ggplot(PAS1000res,aes(x=pheno, y=Prop._h2,fill=pheno)) + geom_bar(stat="identity") + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + theme(legend.position = "none") + annotate("text", label=paste("% of SNPs included", PAS1000_snpsincluded), x=10, y=.15)+labs(x="", title="LD scroe regressino results for 1000 BP around each PAS", y="% of heritability explained")


ggplot(PAS1000res,aes(x=pheno, y=Enrichment, fill=pheno)) + geom_bar(stat="identity") + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + theme(legend.position = "none") + annotate("text", label=paste("% of SNPs included", PAS1000_snpsincluded), x=10, y=12) +labs(x="", title="LD scroe regressino results for 1000 BP around each PAS")
```

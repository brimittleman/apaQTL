...
```{r}
totQTLs_gene= totQTLs %>% inner_join(genes, by="gene")%>% mutate(geneLength=Geneend-Genestart) %>% mutate(dist2QTL=ifelse(strand=="+",as.numeric(SNPend)-as.numeric(Genestart), as.numeric(Geneend)-as.numeric(SNPend)), propGene=dist2QTL/geneLength)  %>% filter(propGene>-5 & propGene<5)
...
Nuclear:
```{r}
nucQTLs_gene= nucQTLs %>% inner_join(genes, by="gene")%>% mutate(geneLength=Geneend-Genestart)%>% mutate(dist2QTL=ifelse(strand=="+",as.numeric(SNPend)-as.numeric(Genestart), as.numeric(Geneend)-as.numeric(SNPend)), propGene=dist2QTL/geneLength)  %>% filter(propGene>-5 & propGene<5)

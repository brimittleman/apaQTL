...
```{r}
totQTLs=read.table("../data/apaQTLs/Total_apaQTLs4pc_5fdr.txt", header = T, stringsAsFactors = F) %>% separate(sid, into=c("SNPchr","SNPloc"), sep=":") %>% mutate(SNPstart=as.integer(SNPloc)-1, SNPend=as.integer(SNPloc), peakID=paste(Gene, Peak, sep=":"),noStrand=".") %>% dplyr::select(SNPchr, SNPstart, SNPend, peakID, slope, noStrand )

write.table(totQTLs, file="../data/apaQTLs/Total_apaQTLs4pc_5fdr.bed", col.names = T, row.names = F, quote = F, sep="\t")

nucQTLs=read.table("../data/apaQTLs/Nuclear_apaQTLs4pc_5fdr.txt", header = T, stringsAsFactors = F) %>% separate(sid, into=c("SNPchr","SNPloc"), sep=":") %>% mutate(SNPstart=as.integer(SNPloc)-1, SNPend=as.integer(SNPloc), peakID=paste(Gene, Peak, sep=":"),noStrand=".") %>% dplyr::select(SNPchr, SNPstart, SNPend, peakID, slope, noStrand )
...
TotalResMProp=melt(TotalRes, id.vars="HMMcat") %>%inner_join(chromHmm, by="HMMcat") %>%  mutate(nQTL= nrow(nucQTLs), propQTL=value/nQTL)%>%  group_by(HMMname) %>% summarise(Mean=mean(propQTL), SD=sd(propQTL)) %>% mutate(Category="Total_APA")
```

...
```{r}
ColocRES=read.table("../data/coloc/AllAssoccolocRes.txt", col.names = c("nsnp",'Neither', 'apaQTL', 'eQTL', 'DifferentSnps', 'Coloc','gene'),stringsAsFactors = F) %>% select(-nsnp)


Coloc_g=ColocRES %>% gather("Hypothesis", "ABF", -gene) 

#Coloc_g$Hypothesis=as.factor(Coloc_g$Hypothesis)
#levels(Coloc_g$Hypothesis)=c("Neither", "apaQTL", "eQTL", "DifferentSnps", "Coloc")
```

```{r}
ggplot(Coloc_g,aes(x=Hypothesis, y=ABF)) +geom_boxplot()
```

```{r}
filter(ColocRES, Coloc > .5 )
```

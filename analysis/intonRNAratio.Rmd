...
```{r}
UpandDown=upstreamMeanDF %>% inner_join(downstreanMeanDF, by="Geneid") %>% mutate(UpMinusDown=UpstreamMean_st-DownstreamMean_st) %>% arrange(desc(UpMinusDown)) %>% separate(Geneid, sep=":", into=c("PAS", "gene", "loc", "PASloc", "Usage"))
...
```
```{r}
write.table(MoreUp, file="../data/intronRNAratio/TotalPAS_MoreUpstreamRNAreads.txt", col.names = T, row.names = F, quote = F, sep="/t" )
```
...
UpandDown_nuc=upstreamNuc %>% inner_join(downstreamNuc, by="Geneid") %>% mutate(UpMinusDown=Upstream_st-Downstream_st) %>% arrange(desc(UpMinusDown)) %>% separate(Geneid, sep=":", into=c("PAS", "gene", "loc", "PASloc", "Usage"))

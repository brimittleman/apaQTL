...
nucspecplot=ggplot(NucBothwSpec, aes(x=Originalslope, y=slope))+geom_point(aes( col=set)) + geom_smooth(method="lm") + labs(title="Nuclear apaQTL effect sizes", x="Effect size in Nuclear",y="Effect size in Total") + geom_density_2d(col="red")+  annotate("text", y=1, x=0, label="R2=.66, slope=0.62") + scale_color_brewer(palette = "Dark2")
```
...
totspecplot=ggplot(TotBothwSpec, aes(x=Originalslope, y=slope))+geom_point(aes( col=set)) + geom_smooth(method="lm") + labs(title="Total apaQTL effect sizes", x="Effect size in Total",y="Effect size in Nuclear") + geom_density_2d(col="red")+  annotate("text", y=1, x=0, label="R2=.62, slope=0.64") + scale_color_brewer(palette = "Dark2")
```
```{r}
plot_grid(nucspecplot,totspecplot)
```

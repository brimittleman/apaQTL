...
For future analysis I want to have a set of PAS with evidence for a signal site.  I want those with 1 of the top 2 used signal sites upstream 10-50 basepairs. The top two use are AATAAA, ATTAAA.  

```{r}
AllsiteDF_1per_use= AllsiteDF_1per %>% dplyr::filter(Site== "AATAAA" | Site== "ATTAAA") %>% filter(Distance2PAS>10, Distance2PAS<50) %>% separate(PAS,into=c("peakid", "loc"),sep="_") %>% separate(peakid,into=c("Peaknum", "gene"),sep=":") %>% mutate(Peak=paste("Peak", Peaknum, sep="")) %>% dplyr::rename("UpstreamDist"=NegCount) %>%  select(Peak, gene, loc, Site, UpstreamDist)
``` 

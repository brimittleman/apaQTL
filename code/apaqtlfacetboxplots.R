...
meltpheno=melt(pheno, id.vars = "chrom", value.name = "Ratio", variable.name = "Individual") %>% separate(Ratio, into=c("num", "denom"), sep="/")  %>%   separate(chrom, into=c("chrom", "start", "end", "peakID"),sep=":") %>% separate(peakID, into=c("gene","loc","strand","PAS"))  %>% mutate(PeakLoc=paste(start, end,loc, sep="\n"))
...
genotypes=read.table(opt$geno, col.names =colnames(genoHead),stringsAsFactors = F ) %>% select(ID,contains("NA"))
...
genoMelt=melt(genotypes, id.vars = "ID", value.name = "FullGeno", variable.name = "Individual" ) %>% select(Individual, FullGeno) %>% mutate(genotype=ifelse(round(as.integer(FullGeno))==0, paste(refAllele, refAllele, sep=""), ifelse(round(as.integer(FullGeno))==1, paste(refAllele,altAllele, sep=""), paste(altAllele,altAllele,sep=""))))
...
qtlplot=ggplot(pheno_qtlpeak, aes(x=genotype, y=PAU, fill=genotype)) + geom_boxplot(width=.5)+ geom_jitter(alpha=1) + facet_grid(~PeakLoc) +scale_fill_brewer(palette = "Dark2")
